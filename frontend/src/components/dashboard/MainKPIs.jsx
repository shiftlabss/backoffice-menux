...
const KPIItem = ({ label, value, subValue, trend, icon: Icon, color, isNegative, onClick }) => (
    <div
        onClick={onClick}
        className={`bg-white p-4 rounded-2xl border border-border hover:shadow-md hover:border-[#D4D4D4] transition-all duration-300 flex flex-col justify-between min-h-[110px] ${onClick ? 'cursor-pointer active:scale-95' : ''}`}
    >
...
export default function MainKPIs() {
    ...
    return (
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6 gap-3 mb-8">
            <KPIItem label="Iniciados" value="215" trend={12} icon={ShoppingBag} color="bg-primary" onClick={() => navigate('/analytics')} />
            <KPIItem label="Finalizados" value="189" trend={8} icon={CheckCircle} color="bg-green-500" onClick={() => navigate('/menu/orders')} />
            <KPIItem label="Conversão" value="42%" trend={2.4} icon={MousePointer2} color="bg-purple-500" onClick={() => navigate('/analytics')} />
            <KPIItem label="Abandono" value="15%" trend={-1.2} isNegative={true} icon={AlertCircle} color="bg-red-500" />

            <KPIItem label="Ticket Médio" value="R$ 92" trend={-1.5} icon={DollarSign} color="bg-emerald-500" onClick={() => navigate('/analytics')} />
            <KPIItem label="TM Categoria" value="R$ 45" subValue="Bebidas" icon={Layers} color="bg-primary" />

            <KPIItem label="T. Decisão" value="4m 12s" trend={-5} isNegative={true} icon={Clock} color="bg-orange-500" onClick={() => navigate('/analytics')} />
            ...
            <KPIItem label="Mesas Ativas" value="24" trend={10} icon={Users} color="bg-primary" onClick={() => navigate('/menu')} />
